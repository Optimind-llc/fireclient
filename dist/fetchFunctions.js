...
function saveCollection(dispatch, path, option, collection) {
    ...
    var docIds = immutable_1.List(collection.map(function (doc) { return pathlib.resolve(path, doc.id); }));
    dispatch({
        type: "setCollection",
        payload: {
...
function getDoc(path, onGet, onError, acceptOutdated) {
    ...
    var _c = provider_1.unwrapContext(provider_1.providerContext), state = _c.state, dispatch = _c.dispatch, firestoreDB = _c.firestoreDB;
